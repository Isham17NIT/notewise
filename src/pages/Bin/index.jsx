...
import { addToHome, deleteFromBin } from "../../slices/notesSlice";
const Bin = ()=>{
    const binContents = useSelector(state=>state.notes.binContents)
    const dispatch = useDispatch();
    const onRestoreClick = (content)=>{
        dispatch(deleteFromBin(content));
                        ...
                        return (
                            <div key={uuid()} className="flex flex-col p-4 min-h-52 max-h-52 rounded-2xl border-1 border-gray-200">
                                <div className="flex">
                                    ...
                                    <RestoreFromTrashIcon className="mr-4" onClick={()=>onRestoreClick(content)}/>
                                </div>
                                <Divider/>
